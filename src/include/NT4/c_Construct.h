...
public:
	...
	void output_Network_Charges()
	{
		...
		std::ofstream OFXY;
		std::ofstream OFRC;
		OF.open("./Testing/Node_Network_Output/node_Charge_Output.ssv", std::ios::app);
		...
		while (tmp_Node != NULL)
		{
			...
			{
				tmp_Top_Tier = tmp_Node->Tier;
			}

			if (tmp_Widest_Tier <= tmp_Node->Tier)
			{
				tmp_Widest_Tier = tmp_Node->Tier;
			}
		...
		for (int cou_T = 0; cou_T < tmp_Top_Tier; cou_T++)
		{
			...
			if (Nodes.Fat_Tier < Skipdex[cou_T]) { Nodes.Fat_Tier = Skipdex[cou_T]; }
		}
